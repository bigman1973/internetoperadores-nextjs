...
export default function CrmContactosDataSyncPanel({ total, initialCompleted }: { total: number; initialCompleted: number }) {
  ...
  if (!canWrite && initialCompleted >= total) return null
  ...
  const run = async () => {
    ...
    const restart = completed >= total && total > 0
    let imported = restart ? 0 : completed
    if (restart) setCompleted(0)
    try {
      for (let attempt = 0; attempt < 300; attempt += 1) {
        ...
        if (Number(data.remaining || 0) === 0) {
          setMessage({ type: 'success', text: `Información completa importada para ${total.toLocaleString('es-ES')} contactos. Ya puedes consultar y ampliar todos los campos disponibles.` })
          setRunning(false)
          return
        }
      }
    ...
    <section className="rounded-xl border border-blue-200 bg-blue-50 p-5 sm:p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-2 text-xs font-semibold text-blue-900">{completed.toLocaleString('es-ES')} de {total.toLocaleString('es-ES')} contactos · {percent}%</p>
        </div>
        {canWrite && <button type="button" onClick={run} disabled={running} className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-lg bg-blue-700 px-5 py-2.5 text-sm font-semibold text-white hover:bg-blue-800 disabled:opacity-60"><ArrowPathIcon className={`h-5 w-5 ${running ? 'animate-spin' : ''}`} />{running ? 'Importando…' : completed >= total && total > 0 ? 'Actualizar toda la información' : completed > 0 ? 'Continuar importación' : 'Importar información completa'}</button>}
      </div>
