...
interface Contrato { id: string; titulo: string; codigoContrato: string | null; tipo: string; horasContratadas: number; horasMes: number; saldoContrato: number; horasImputadasActualizaciones: number; horasImputadasConFactor: number; horasDisponibles: number; precioHoraContrato: number | null }
        ...
        {tab === 'imputacion' && (
          <div>
            ...
            <div className="bg-white border rounded-lg p-4 mb-4">
              ...
              <table className="w-full text-sm">
                <thead className="border-b"><tr><th className="text-left py-1 text-[10px] text-gray-500 uppercase">Contrato</th><th className="text-right py-1 text-[10px] text-gray-500 uppercase">Horas/mes</th><th className="text-right py-1 text-[10px] text-gray-500 uppercase">Saldo contrato</th><th className="text-right py-1 text-[10px] text-gray-500 uppercase">Imput. actualiz. (x{kpis?.factorVigente || 1})</th><th className="text-right py-1 text-[10px] text-gray-500 uppercase">Disponible neto</th></tr></thead>
                <tbody>
                  {contratos.map(c => (
                    <tr key={c.id} className="border-b">
                      <td className="py-2 text-gray-700">{c.titulo}</td>
                      <td className="py-2 text-right">{c.horasMes}h</td>
                      <td className={`py-2 text-right font-medium ${c.saldoContrato >= 0 ? 'text-blue-600' : 'text-red-600'}`}>{c.saldoContrato}h</td>
              ...
              <p className="text-[9px] text-gray-400 mt-2">Saldo contrato = horas cubiertas por personal - horas comprometidas (dato del seguimiento). Disponible neto = saldo contrato - horas imputadas de actualizaciones (x factor).</p>
            </div>
