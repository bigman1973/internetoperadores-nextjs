...
export default function LeadsMantenimientoPage() {
  ...
  const handleDeleteLead = async (id: string, nombre: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/leads-mantenimiento/${id}`, { method: 'DELETE' });
      if (res.ok) {
        fetchLeads();
      } else {
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="inline-flex items-center gap-1 px-2 py-1.5 text-red-600 hover:bg-red-50 rounded-lg text-xs transition-all"
                            title="Eliminar lead"
                          >
