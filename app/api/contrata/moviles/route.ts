...
export async function POST(request: Request) {
  try {
    ...
    const { nombre, email, empresa, telefono, numLineas, operadorActual, consumoDatos, permanenciaActual, tipoDispositivos, comentarios } = body;
    ...
    const emailComercialHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background: #1a1a2e; padding: 20px; text-align: center;">
          <h1 style="color: #f97316; margin: 0; font-size: 20px;">Nueva Solicitud de Soluciones Móviles</h1>
        ...
        <div style="padding: 24px; background: #f9f9f9;">
          ...
          <div style="margin-top: 16px; padding: 16px; background: white; border-radius: 8px; border: 1px solid #e5e7eb;">
            <h3 style="margin: 0 0 12px 0; color: #f97316; font-size: 16px;">Datos de telefonía móvil</h3>
            <table style="width: 100%; border-collapse: collapse;">
              <tr><td style="padding: 8px; font-weight: bold; color: #374151;">Nº líneas necesarias:</td><td style="padding: 8px; color: #1f2937;">${numLineas || 'No indicado'}</td></tr>
              ...
              <tr><td style="padding: 8px; font-weight: bold; color: #374151;">Consumo medio datos:</td><td style="padding: 8px; color: #1f2937;">${consumoDatos || 'No indicado'}</td></tr>
              <tr style="background: #f9fafb;"><td style="padding: 8px; font-weight: bold; color: #374151;">Permanencia actual:</td><td style="padding: 8px; color: #1f2937;">${permanenciaActual || 'No indicada'}</td></tr>
    ...
    const emailInteresadoHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background: #1a1a2e; padding: 20px; text-align: center;">
          ...
          <p style="color: #9ca3af; margin: 8px 0 0 0; font-size: 14px;">Soluciones Móviles</p>
        </div>
        <div style="padding: 24px;">
          <p style="color: #4b5563; line-height: 1.6;">Hola <strong>${nombre}</strong>,</p>
          <p style="color: #4b5563; line-height: 1.6;">Hemos recibido tu solicitud de <strong>Soluciones Móviles</strong> para <strong>${empresa}</strong>.</p>
          ...
          <div style="background: #f9fafb; padding: 16px; border-radius: 8px; margin: 20px 0;">
            ...
            <ul style="margin: 0; padding-left: 20px; color: #4b5563; line-height: 1.8;">
              ...
              <li>Tipo dispositivos: <strong>${tipoDispositivos || 'No indicado'}</strong></li>
            </ul>
          </div>
          <div style="background: #fff7ed; border: 2px solid #f97316; border-radius: 12px; padding: 20px; margin: 24px 0; text-align: center;">
            ...
            <ul style="margin: 12px 0 0 0; padding-left: 20px; color: #4b5563; text-align: left; line-height: 1.8;">
              <li>Comparativa de tarifas móviles empresariales</li>
              <li>Análisis de cobertura en tus zonas de trabajo</li>
              <li>Opciones de financiación de terminales</li>
              <li>Gestión centralizada de todas las líneas</li>
              <li>Ahorro estimado vs tu operador actual</li>
            </ul>
          </div>
    ...
    // Enviar emails
    const emailPromises = [
      sendBrevoEmail({ email: 'comercial@internetoperadores.com', name: 'Comercial IO' }, `Nueva solicitud Soluciones Móviles - ${empresa} (${numLineas || 'N/A'} líneas)`, emailComercialHtml),
      sendBrevoEmail({ email: 'victor@internetoperadores.com', name: 'Victor' }, `Nueva solicitud Soluciones Móviles - ${empresa} (${numLineas || 'N/A'} líneas)`, emailComercialHtml),
      sendBrevoEmail({ email, name: nombre }, `Tu solicitud de Soluciones Móviles ha sido recibida - Internet Operadores`, emailInteresadoHtml),
    ];

    // HubSpot: lista Móviles + Newsletter Empresas
    const notaHubspot = `Solicitud de Soluciones Móviles\n- Líneas necesarias: ${numLineas || 'No indicado'}\n- Operador actual: ${operadorActual || 'No indicado'}\n- Consumo datos: ${consumoDatos || 'No indicado'}\n- Permanencia: ${permanenciaActual || 'No indicada'}\n- Tipo dispositivos: ${tipoDispositivos || 'No indicado'}\n- Comentarios: ${comentarios || 'Sin comentarios'}\n\nFecha: ${new Date().toLocaleString('es-ES', { timeZone: 'Europe/Madrid' })}`;
